...
int main(int argc, char *argv[])
{
	...
	/*根据不同的路径与关键字产生标准的key*/
	if ((key_snd_msg = ftok(".", '2')) == -1)
	{
		perror("ftok");
		return -1;
	}

	if ((key_rcv_msg = ftok(".", '1')) == -1)
	{
		perror("ftok");
		return -1;
	}
	...
	/*打开消息队列*/
	if ((qid_snd_msg = msgget(key_snd_msg, 0666)) == -1)
	{
		perror("msgget");
		return -1;
	}	

	if ((qid_rcv_msg = msgget(key_rcv_msg, 0666)) == -1)
	{
		perror("msgget");
		return -1;
	}	
	...
	/*创建接收消息线程*/
	if(pthread_create(&recv_pthread_id, NULL, (void *)pthread_rcv_msg, NULL) != 0)
	{
		printf("create msgrcv thread fail\n");
	}

	/*等待线程结束，回收线程资源*/
	pthread_join(recv_pthread_id, NULL);
